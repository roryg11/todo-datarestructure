class TodoApp < CommandLineApp
  attr_accessor :input, :output, :projects_with_tasks, :view

  def initialize(input, output)
    @input = input
    @output = output
    @projects_with_tasks = {}
    @view = View.new(input, output, projects_with_tasks)
  end
  ...
  def task_menu
    ...
    view.edit_projects_tasks_prompt
    # view.print_projects(@projects_with_tasks)
    project = gets.chomp
    ...
    while task_menu
      ...
      if @projects_with_tasks.has_key?(project)
        view.print_task_menu(project)
      end

      task_input = gets.chomp
      if task_input == 'list'
        view.task_lister(project)
      elsif task_input == 'create'
        view.create_task_tasks_prompt
        create_tasks(project)
      elsif task_input == 'edit'
        view.edit_task_tasks_prompt
        edit_task(project)
      elsif task_input == 'complete'
        view.task_completed_tasks_prompt(project)
